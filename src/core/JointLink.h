...
#include "DhParam.h"
#include "../meshes/JointLinkMesh.h"
#include "PidController.h"

//describes a combination of a joint and a link
class JointLink {
    public:
        JointLink(DhParam params, float joint_rotation, int link_number);
        ...
        //setters
        void setDhParams(const DhParam& params){ 
            params_ = params; 
            recreateMesh(); 
        }
        void setToWorld(const Eigen::Affine3f& to_world){
            to_world_ = to_world;
        }
        void setJointRotation(float rotation_angle){
            rotation_ = rotation_angle;
        }
        void setJointTargetRotation(float target_angle){
            target_rotation_ = target_angle;
        }
        void setControllerGains(float p, float i, float d){
        ...
        float getJointRotations() const { return rotation_; }
    ...
    private:
        ...
        int link_number_; //the ordinal number of this link in the robot arm's link chain, starting from the base
        JointLinkMesh mesh_; //mesh used for graphical representation
        PidController controller_; //controls the joint

        float joint_speed_; //current speed of joint in rad/s
        float rotation_; //rotation angle of the joint
        float target_rotation_; //target rotation for the joint, current roation will move towards this when update() is called
        ...
        //transforms the coordinates of this link fram to world frame
