    ...
    length_(length)
{
    ...
    for(int i=0; i<num_ring_verts; i++) {
        //make two triangles in the rectangle formed by two pairs of vertices on the opposite faces
        Vertex v1, v2, v3, v4;
        v1 = ring1[i];
...
std::vector<Vertex> CylinderMesh::createFaceTriangles(const std::vector<Vertex>& ring_vertices, float z) const{
    ...
    //for each pair of ring vertices, create trinalge from it to the center vertex
    for(int i=0; i<ring_vertices.size(); i++){
