#include "JointLinkMesh.h"
#include "CylinderMesh.h"
#include "BoxMesh.h"

using namespace Eigen;

const Vector4f JointLinkMesh::JOINT_COLOR = Vector4f(0.05, 0.6, 0.4, 1.0);
const Vector4f JointLinkMesh::LINK_COLOR = Vector4f(0.9, 0.5, 0, 1.0);

JointLinkMesh::JointLinkMesh(float z_len, float x_len, int link_number) : Mesh() {
    float init_link_thickness = 0.1f;
    float init_joint_radius = sqrt(2*(init_link_thickness/2)*(init_link_thickness/2)) + 0.001;
    float init_joint_length = 0.02f;
    ...
    //if both links exist, make small adjustment to the lengths so that they interlock properly
    if(both_directions) {
    ...
    //z-link starts from origin and ends at z_len along z
